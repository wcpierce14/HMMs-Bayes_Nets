...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        selection = random.choices(keys, weights = vals, k = 1)[0]
        return selection
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # If sensor reads None, check if ghost position is in jail
        if noisyDistance == None:
            if ghostPosition == jailPosition: return 1
            else: return 0

        # If ghost is in jail but sensor does not read None, return a probability of 0
        if ghostPosition == jailPosition: return 0

        # If we dont meet the boundary conditions above, compute probability directly
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            probObs = self.getObservationProb(observation, pacmanPos, pos, jailPos)
            self.beliefs[pos] *= probObs
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newProbs = DiscreteDistribution()
        for pos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, pos)
            for key in newPosDist.keys():
                loc = key
                prob = newPosDist[loc]
                newProbs[loc] += (prob * self.beliefs[pos])

        # newProbs.normalize()
        self.beliefs = newProbs
