...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            currDist = livingGhostPositionDistributions[i]
            bestPos = currDist.argMax()
            distToGhost = self.distancer.getDistance(bestPos, pacmanPosition)
            if distToGhost < closestGhost:
                closestGhost = distToGhost
                temp = float('inf')
                for action in legal:
                    successorPosition = Actions.getSuccessor(pacmanPosition, action)
                    newDist = self.distancer.getDistance(bestPos, successorPosition)
                    if newDist < temp:
                        temp = newDist
                        bestAction = action
        return bestAction            
